...
type HistoryEntry = ProductUsage & { active: boolean };

const EMPTY_USAGE: UsageDraft = { workOrder: '', aircraft: '', operator: '' };
...
export default function ProductUsagePanel({ product, onUpdated, onMessage }: Props) {
  ...
  const historyEntries = useMemo<HistoryEntry[]>(() => {
    const entries: HistoryEntry[] = (product.usageHistory ?? []).map((usage) => ({ ...usage, active: false }));
    if (inUse && product.currentUsage) entries.push({ ...product.currentUsage, returnedAt: undefined, active: true });
    return entries.sort((first, second) => new Date(second.startedAt).getTime() - new Date(first.startedAt).getTime());
  ...
  useEffect(() => {
    ...
    const closeWithEscape = (event: KeyboardEvent) => { if (event.key === 'Escape') setHistoryOpen(false); };
  ...
  async function startUsage(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await saveProduct({
        ...
        currentUsage: { workOrder, aircraft, operator, startedAt: now },
        updatedAt: now,
      });
      await onUpdated();
  ...
  async function returnToStock() {
    ...
    const now = new Date().toISOString();
    const completedUsage: ProductUsage = { ...product.currentUsage, returnedAt: now };
    setSaving(true);
    setError('');
    try {
      await saveProduct({
  ...
  const historyModal = historyOpen && typeof document !== 'undefined'
    ? createPortal(
        <div className="usage-history-backdrop" onMouseDown={closeFromBackdrop}>
          <section className="usage-history-modal" role="dialog" aria-modal="true" aria-labelledby={`usage-history-title-${product.id}`}>
            <header className="usage-history-modal-header">
              ...
              <button type="button" className="usage-history-close-icon" onClick={() => setHistoryOpen(false)} aria-label="Fechar histórico de uso">×</button>
            </header>
            <div className="usage-history-modal-summary"><span>{historyEntries.length} registro(s)</span><small>Mais recente primeiro</small></div>
              ...
              {historyEntries.map((usage, index) => (
                <article className={`usage-history-entry ${usage.active ? 'active' : 'completed'}`} key={`${usage.startedAt}-${index}`}>
                  <div className="usage-history-entry-heading">
                    <div><span>REGISTRO {historyEntries.length - index}</span><strong>{usage.operator}</strong></div>
                    <span className={`usage-history-status ${usage.active ? 'active' : 'completed'}`}>{usage.active ? 'Em uso' : 'Finalizado'}</span>
                  </div>
                  <dl className="usage-history-entry-grid">
                    ...
                    <div className="usage-history-return-time"><dt>Data e hora da devolução</dt><dd>{usage.active ? 'Material ainda em uso' : formatDateTime(usage.returnedAt)}</dd></div>
                  </dl>
                </article>
              ))}
            </div>
            <footer className="usage-history-modal-footer"><button type="button" onClick={() => setHistoryOpen(false)}>Fechar histórico</button></footer>
          </section>
        </div>,
        document.body,
      )
  ...
  return (
    <>
      ...
      <StockQuantityControl product={product} onUpdated={onUpdated} onMessage={onMessage} />

      <section className={`usage-panel ${inUse ? 'is-in-use' : 'is-in-stock'}`}>
        <div className="usage-panel-heading">
          <div><span className="usage-kicker">SITUAÇÃO DO MATERIAL</span><strong>{inUse ? 'Em uso' : 'Disponível no estoque'}</strong></div>
        ...
        {inUse && product.currentUsage ? (
          <>
            ...
            <button className="usage-return-button" type="button" onClick={() => void returnToStock()} disabled={saving}>{saving ? 'Registrando...' : 'Retornar ao estoque'}</button>
          </>
        ) : formOpen ? (
          <form className="usage-form" onSubmit={startUsage}>
            ...
            <label><span>OM *</span><input value={draft.workOrder} onChange={(event) => setDraft((current) => ({ ...current, workOrder: event.target.value }))} placeholder="Ex.: OM 45872" autoFocus /></label>
            <label><span>Avião *</span><input value={draft.aircraft} onChange={(event) => setDraft((current) => ({ ...current, aircraft: event.target.value }))} placeholder="Ex.: PR-ABC ou MSN 1234" /></label>
            <label><span>Operador *</span><input value={draft.operator} onChange={(event) => setDraft((current) => ({ ...current, operator: event.target.value }))} placeholder="Nome ou matrícula" /></label>
            {error && <p className="usage-error" role="alert">{error}</p>}
            <div className="usage-form-actions">
              <button type="button" className="usage-cancel-button" onClick={() => { setFormOpen(false); setDraft(EMPTY_USAGE); setError(''); }} disabled={saving}>Cancelar</button>
              <button type="submit" className="usage-start-button" disabled={saving}>{saving ? 'Registrando...' : 'Confirmar retirada'}</button>
            </div>
          </form>
        ) : (
          <button className="usage-start-button full" type="button" onClick={() => setFormOpen(true)}>Retirar material para uso</button>
        )}

        {!inUse && error && <p className="usage-error" role="alert">{error}</p>}
        {historyEntries.length > 0 && (
          <button className="usage-history-open-button" type="button" onClick={() => setHistoryOpen(true)}><span>Ver histórico de uso ({historyEntries.length})</span><span aria-hidden="true">↗</span></button>
        )}
      </section>
      {historyModal}
    </>
